...
Publish the repository root from the `main` branch in **Settings → Pages**. All application
URLs are relative, so the same files work for both a user site such as
`https://username.github.io/` and a project site such as
`https://username.github.io/repository-name/`.

After updating an existing deployment, reload once while holding Shift or clear the old
service worker in Safari's developer tools so the previous cached shell is removed.
